...
func (r *Repository) GetAll(limit int, offset int, searchText string) ([]models.Topic, error) {
	var topics []models.Topic
	var query string
	if limit == 0 && len(searchText) > 0 {
		query = `SELECT topic_id, Username, title, body, reported, created_date, updated_date FROM topics WHERE title ILIKE '%' || $1 || '%' ORDER BY created_date DESC`
	} else if limit == 0 {
...
func (r *Repository) fetch(query string, limit int, offset int, searchText string) ([]models.Topic, error) {
	var rows *sql.Rows
	var err error
	if limit != 0 {
		rows, err = r.dbClient.Query(query, limit, offset)
	} else if len(searchText) > 0 {
		rows, err = r.dbClient.Query(query, searchText)
	} else {
